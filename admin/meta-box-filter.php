...
function apl_render_categories( $post_type, $taxonomy, $term_parent = 0, $indent = 0 ) {
	...
	?>
	<?php if ( ! empty( $terms ) ) : $e_indent = 18 * $indent; ?>
		<ul style="margin-left: <?php echo $e_indent; ?>px;">
			<?php if ( 0 === $term_parent ) : $ele_tag = $post_type . '-' . $taxonomy . '-any'?>
				<li>
					<label for="term-<?php echo $ele_tag; ?>">
						<input type="checkbox" id="term-<?php echo $ele_tag; ?>" name="term-<?php echo $ele_tag; ?>" >
						<?php esc_html_e( 'Any / All Terms', 'advanced-post-list' ); ?>
					</label>
				</li>
			<?php endif; ?>
			<?php foreach ( $terms as $k_index => $v_term_obj ) : $ele_tag = $post_type . '-' . $taxonomy . '-' . $v_term_obj->slug; ?>
				<li>
					<label for="term-<?php echo $ele_tag; ?>">
						<input type="checkbox" id="term-<?php echo $ele_tag; ?>" name="term-<?php echo $ele_tag; ?>" >
						<?php echo $v_term_obj->name; ?>
					</label>
				</li>
...
function apl_render_page_parents( $post_type, $page_parent = 0, $indent = 0 ) {
	...
	<?php while ( $query_pages->have_posts() ) : $query_pages->the_post(); ?>
				<tr class="alternate">
					<td class="apl-chk-POST_TYPE-PAGE">
						<input type="checkbox" class="apl-page-post_type-pages" id="apl-page-<?php echo $post_type; ?>-<?php echo $query_pages->post->post_name; ?>" name="apl-page-<?php echo $post_type; ?>-<?php echo $query_pages->post->post_name; ?>" />
					</td>
					<td>
						<?php echo $query_pages->post->ID; ?>
					</td>
					<td class="row-title">
						<label for="apl-page-<?php echo $post_type; ?>-<?php echo $query_pages->post->post_name; ?>"><?php echo $indent_str . ' ' . $query_pages->post->post_title; ?></label>
					</td>
				</tr>
				<?php
...
<div class="apl-filter-box-1">
	<div class="apl-left-minibar">
		...
		<div class="apl_post_types">
			<?php foreach ( $apl_post_tax as $k_pt_slug => $v_arr ) : ?>
				<input type="checkbox" class="apl-toggle-post_type" id="apl-toggle-<?php echo $k_pt_slug; ?>" name="apl-toggle-<?php echo $k_pt_slug; ?>" >
				<label for="apl-toggle-<?php echo $k_pt_slug; ?>"><?php echo $v_arr['name']; ?></label>
				<br>
			<?php endforeach; ?>
		</div>
	</div>
	<div id="apl-filter-tax-query" class="apl-filter-post-tax-query">
		...
		<?php foreach ( $apl_post_tax as $k_pt_slug => $v_pt_arr ) : ?>
			<div id="apl-filter-<?php echo $k_pt_slug ?>" class="apl-filter-post-type">
				<h4><span><?php echo $v_pt_arr['name']; ?></span></h4>
				<div id="apl-tabs-<?php echo $k_pt_slug; ?>-type" class="apl-tabs-post_type-type">
					<ul>
						<?php if ( ! empty( $v_pt_arr['tax_arr'] ) ) : ?>
							<li class="apl-t-li-post-type"><a href="#apl-t-<?php echo $k_pt_slug; ?>-type-taxonomies"><h5><?php esc_html_e( 'Taxonomies', 'advanced-post-list' ); ?></h5></a></li>
						<?php endif; ?>
						<?php if ( isset( $apl_post_type_objs[ $k_pt_slug ] ) ) : ?>
							<?php if ( $apl_post_type_objs[ $k_pt_slug ]->hierarchical ) : ?>
								<li class="apl-t-li-post-type"><a href="#apl-t-<?php echo $k_pt_slug; ?>-type-pages"><h5><?php esc_html_e( 'Parent Pages', 'advanced-post-list' ); ?></h5></a></li>
							<?php endif; ?>
						<?php endif; ?>
					</ul>
					<!-- TAXONOMIES -->
					<?php if ( ! empty( $v_pt_arr['tax_arr'] ) ) : ?>
						<div id="apl-t-<?php echo $k_pt_slug; ?>-type-taxonomies">
							<div class="apl-pt-taxonomy-selector">
								<select id="apl-multiselect-<?php echo $k_pt_slug; ?>" name="apl-multiselect-<?php echo $k_pt_slug; ?>" value="<?php echo $k_pt_slug; ?>" multiple="multiple">

									<?php foreach ( $v_pt_arr['tax_arr'] as $index => $tax_slug ) : ?>
										<?php $opt_value = $k_pt_slug . '-' . $tax_slug; ?>
										<option value="<?php echo $opt_value; ?>"><?php echo $apl_taxonomy_objs[ $tax_slug ]->labels->singular_name; ?></option>
									<?php endforeach; ?>
								</select>
							</div>
							<div class="apl-pt-taxonomy-tabs">
								<div id="apl-tabs-<?php echo $k_pt_slug; ?>-taxonomies" class="apl-tabs-post-type-taxonomies">
									<ul>
										<?php foreach ( $v_pt_arr['tax_arr'] as $index => $tax_slug ) : ?>
											<li id="apl-t-li-<?php echo $k_pt_slug; ?>-<?php echo $tax_slug; ?>"><a href="#apl-t-div-<?php echo $k_pt_slug; ?>-<?php echo $tax_slug; ?>"><?php echo $apl_taxonomy_objs[ $tax_slug ]->labels->singular_name; ?></a></li>
										<?php endforeach; ?>
									</ul>
									<?php foreach ( $v_pt_arr['tax_arr'] as $index => $tax_slug ) : ?>
										<div id="apl-t-div-<?php echo $k_pt_slug; ?>-<?php echo $tax_slug; ?>" class="apl-t-terms">
											<!-- WP_Terms -->
											<?php $ele_tag = $k_pt_slug . '-' . $tax_slug?>
											<ul>
												<li>
													<label for="apl_chk_req_tax_<?php echo $ele_tag; ?>">
														<input type="checkbox" id="apl_chk_req_tax_<?php echo $ele_tag; ?>" name="check_required_taxonomy_<?php echo $ele_tag; ?>" >
														<?php echo __( 'Require this Taxonomy.', 'advanced-post-list' ); ?>
													</label>
												</li>
												<li>
													<label for="apl_chk_req_terms_<?php echo $ele_tag; ?>">
														<input type="checkbox" id="apl_chk_req_terms_<?php echo $ele_tag; ?>" name="apl_chk_req_terms_<?php echo $ele_tag; ?>" >
														<?php echo __( 'Require added Terms', 'advanced-post-list' ); ?>
													</label>
												</li>
												<li>
													<label for="apl_chk_dynamic_<?php echo $ele_tag; ?>">
														<input type="checkbox" id="apl_chk_dynamic_<?php echo $ele_tag; ?>" name="check_dynamic_<?php echo $ele_tag; ?>" >
														<?php echo __( 'Dynamic Terms', 'advanced-post-list' ); ?>
													</label>
												</li>
											</ul>
					...
					<?php if ( isset( $apl_post_type_objs[ $k_pt_slug ] ) ) : ?>
						<?php if ( $apl_post_type_objs[ $k_pt_slug ]->hierarchical ) : ?>
							<div id="apl-t-<?php echo $k_pt_slug; ?>-type-pages">
				...
				<div>
					...
					<select id="apl_author_ids" class="apl-multiselect-author-ids">
						...
						<?php foreach ( $wp_roles->role_names as $k_role_slug => $v_role_name ) : ?>
							...
							?>
							<?php if ( $apl_authors ) : ?>
								<optgroup label="<?php echo $v_role_name; ?>">
									<?php foreach ( $apl_authors as $k2_index => $v2_user ) : var_dump( $v2_user ); ?>
										<option value="<?php echo $v2_user->data->ID; ?>" ><?php echo $v2_user->data->display_name; ?></option>
									<?php endforeach; ?>
								</optgroup>
							<?php endif; ?>
						<?php endforeach; ?>
